...
<h3 style="text-align: center;">{{ $voucher->type }} {{ __('VOUCHER')}}</h3>
...
<table class="invoice-info-container">
    ...
    <tr>
        <td>
            {{ __('Voucher Date:')}} <strong>{{ $voucher->voucher_date?->format('M d,Y') }}</strong><br>
            {{ __('Voucher Number:')}} <strong>{{ $voucher->voucher_id }}</strong>

        </td>
        <td>
            {{ __('Property:')}} <strong>{{ $voucher->property?->name }}</strong><br>
            {{ __('House:')}} <strong>{{ $voucher->house?->name }}</strong>
        </td>
    </tr>
...
<table class="line-items-container has-bottom-border">
    ...
    <tr>
        ...
        <td class="large"></td>
        <td class="large total">{{setting('currency_symbol').' '.number_format($voucher->items->sum('cost'),2) }}</td>
    </tr>
    </tbody>
</table>
