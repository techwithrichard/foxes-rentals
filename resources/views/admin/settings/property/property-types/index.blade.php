                                                ...
                                                <div class="toggle-expand-content" data-content="pageMenu">
                                                    <ul class="nk-block-tools g-3">
                                                        <li class="nk-block-tools-opt">
                                                            <a href="{{ route('admin.settings.property.types.create') }}" class="btn btn-primary">
                                                    ...
                                                    <tr data-type-id="{{ $propertyType->id }}" data-status="{{ $propertyType->is_active ? 'active' : 'inactive' }}">
                                                        ...
                                                        <td>
                                                            <div class="btn-group" role="group">
                                                                <a href="{{ route('admin.settings.property.types.edit', $propertyType) }}" class="btn btn-sm btn-outline-secondary" title="{{ __('Edit') }}">
...
function toggleStatus(propertyTypeId) {
    fetch(`{{ url('admin/settings/property/types') }}/${propertyTypeId}/toggle-status`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
            'Content-Type': 'application/json',
...
function deletePropertyType(propertyTypeId) {
    ...
    fetch(`{{ url('admin/settings/property/types') }}/${propertyTypeId}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        }
