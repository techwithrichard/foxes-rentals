                                    ...
                                    <div class="nk-block-head nk-block-head-lg">
                                        <div class="nk-block-between">
                                            <div class="nk-block-head-content">
                                                <h5 class="nk-block-title">{{ __('User Profile Settings') }}</h5>
                                                <p>{{ __('Configure user profile fields and requirements') }}</p>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="nk-block">
                                        @if($profileGroup && $profileGroup->items->count() > 0)
                                            <div class="card">
                                                <div class="card-body">
                                                                                ...
                                                                                <select class="form-select" 
                                                                                    ...
                                                                                    @if($item->options)
                                                                                        @foreach($item->options as $value => $label)
                                                                                            <option value="{{ $value }}" {{ in_array($value, explode(',', $item->value)) ? 'selected' : '' }}>
                                                                                                {{ $label }}
                                                                                            </option>
                                                                                        @endforeach
                                                                                    @endif
                ...
                form.addEventListener('change', function(e) {
                    if (e.target.hasAttribute('data-key')) {
                        ...
                        if (e.target.type === 'checkbox') {
                            ...
                            value = Array.from(e.target.selectedOptions).map(option => option.value).join(',');
                        }
